...
class myHandler(SimpleHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if self.path == '/do_signup':
            data = self.rfile.read(int(self.headers.get('Content-typet-Length')))
        ...
        elif self.path == '/do_crop':
            data = self.rfile.read(int(self.headers.get('Content-Length')))
            data = json.loads(data)
            self.db_connection.create_crop(data)

   def do_GET(self):
        if self.path in ['/', '/signup', '/signin', '/crop']:
...
def start_server():
    ...
    httpd = HTTPServer(('0.0.0.0', 3611), myHandler)
    httpd.serve_forever()
